...
"""
generate rpm spec for python from github

Usage:
  pyrpmspec python <git_hub_url> <version> [--rpmbuild_root RBROOT]
...
def gen_spec(args):
    spec_template = 'python-template.spec'
    ...
    spec_filename = 'python-'+repo_name+'.spec'
    ...
    spec_script.append('rm -f $RPMDIR/python-'+repo_name+'-*')
    spec_script.append('mv -f $RPMBUILDROOT/RPMS/x86_64/python-'+repo_name+'-* $RPMDIR')
    spec_script.append('mv -f $RPMBUILDROOT/RPMS/noarch/python-'+repo_name+'-* $RPMDIR')
    print('\n'.join(spec_script))
    return None
